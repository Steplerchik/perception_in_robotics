...
def get_cli_args():
    ...
    parser.add_argument('-o',
                        ...
                        default=None,
                        action='store',
                        help='The output directory to which the '
    ...
    with movie_writer.saving(fig, args.movie_file, data.num_steps) if should_write_movie else get_dummy_context_mgr():
        for t in range(data.num_steps):
            ...
            for i in range(0, len(slam_filter.ld_est), 2):
                plot2dcov(slam_filter.ld_est[i:i+2], slam_filter.Sigma_ld[:, :, i//2],
                          'green', 3
                          )
            if tp1 == 1:
                print('\nA for t=1:\n', np.round(slam_filter.A, decimals=1))

            plot2dcov(slam_filter.mu_est[-3:-1], slam_filter.Sigma[:-1, :-1, -1],
                      'blue', 3,
